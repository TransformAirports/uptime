...
// Load Postmark API key from a local JSON file
const postmarkConfigPath = path.join(__dirname, 'postmarkConfig.json');
const postmarkConfig = JSON.parse(fs.readFileSync(postmarkConfigPath, 'utf8'));
const postmarkApiKey = postmarkConfig.apiKey;
...
exports.updateUptime = functions.https.onRequest(async (req, res) => {
  if (req.method !== 'POST') {
...
exports.calculateUptime = functions.pubsub.topic("calculate-uptime").onPublish(async (message) => {
  const now = new Date();
